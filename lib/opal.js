...
// tmp replace path
Opal.opal_lib_path = path.join(path.dirname(__filename), '..', 'opal_lib');
...
if (require.extensions) {
  require.extensions['.rb'] = function(module, filename) {
    ...
    console.log(result);
    console.log("and doneee");
    return module._compile('(function(self) {' + result + '})(rb_top_self);', filename);
  };
} else if (require.registerExtension) {
  require.registerExtension('.rb', function(source) {
    console.log("and done?");
    return '(function(self) {' + exports.compile(source) + '})(rb_top_self);';
  });
}
