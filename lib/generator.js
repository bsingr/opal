...
MainIseq.prototype.join = function() {
  ...
  res.push('Qtrue = Opal.Qtrue, Qfalse = Opal.Qfalse, $range = Opal.G, $block = Opal.P, ');
  res.push('$cg = Opal.cg;\n');
...
DefIseq.prototype.method_fixing = function(res) {
  ...
  // handle rest args
  if (rest) {
    var rest_offset = norm + opt ; // should take into account opt.. we add one to skip self
...
RubyGenerator.prototype = {
     ...
     //   var tmp_recv = this.iseq_current.temp_local();
       // pre.push('(' + tmp_recv + ' = ' + this.generate(stmt[1]));
        ...
        if (stmt[4]) {
          ...
          pre.push('$block.f = ' + tmp_recv + this.mid_to_jsid(stmt[2]) + ')');
          pre.push('.call');
        ...
        // no block
          ...
          pre.push(this.generate(stmt[1]));
          pre.push(this.mid_to_jsid(stmt[2]));
        }
        ...
        if (stmt[4]) {
          pre.push('(($block.p = ' + this.generate_block(stmt[4]) + ').$self = ' + this.SELF + ', ');
          pre.push('$block.f = ' + this.SELF + this.mid_to_jsid(stmt[2]) + ')');
          pre.push('.call');
  ...
  generate_colon2: function(stmt) {
    return '$cg(' + this.generate(stmt[1]) + ', "' + stmt[2] + '")';
  },
  ...
  generate_yield: function(stmt) {
    ...
    if (stmt[1][1]) {
      ...
      return block + '.apply(' + block + '.$self' + ', [' + args_res.join('') + ']' +
          '.concat(' + this.generate(stmt[1][1]) + '))';
    }
    else {
      // no splat
      return block + '.call(' + block + '.$self' + args_res.join('') + ')';
    }
    // res.push(')');
