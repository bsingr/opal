...
StatementsNode.prototype.returns = function() {
  if (this.nodes.length > 0) {
    this.nodes[this.nodes.length - 1] = new ReturnNode(this.nodes[this.nodes.length - 1]);
  } else {
    // this.nodes[0] = (new ReturnNode(new NilNode({})));
  }
};
...
CallNode.prototype.generate = function(opts) {
  ...
  if (this.recv instanceof NumericNode) {
    code += ('(' + this.recv.process(opts, LEVEL_EXPR) + ')');
  } else if (this.recv) {
    code += this.recv.process(opts, LEVEL_EXPR);
  } else {
  ...
  // mid
  code += this.mid_to_jsid(this.mid);
...
IfNode.prototype.generate = function(opts) {
  ...
  for (var i = 0; i < this.tail.length; i++) {
    ...
    if (tail[0].value == 'elsif') {
      ...
      opts.indent += INDENT;
      code += tail[2].process(opts, LEVEL_TOP);

    } else {
      done_else = true;
      code += '} else {';
      opts.indent += INDENT;
      code += tail[1].process(opts, LEVEL_TOP);
    }
  }
...
AssignNode.prototype.generate = function(opts) {
  ...
  if (this.lhs instanceof IvarNode) {
    return (new SelfNode({})).generate(opts) + '["' + this.lhs.name + '"] = ' + this.rhs.generate(opts);
  } else if (this.lhs instanceof IdentifierNode) {
    return this.lhs.name + ' = ' + this.rhs.generate(opts);
    ...
    throw new Error("bad lhs for assign");
  }
};
...
var ReturnNode = exports.ReturnNode = function(val) {
  this.line = val.line;
  this.val = val;
};
...
ReturnNode.prototype.generate = function(opts) {
