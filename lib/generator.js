...
DefIseq.prototype.method_fixing = function(res) {
  ...
  // handle rest args
  if (rest) {
    var rest_offset = norm + opt + 0; // should take into account opt.. we add one to skip self
...
RubyGenerator.prototype = {
  ...
  generate_call: function(stmt) {
    ...
    // if (stmt[1]) { // recv
        ...
        if (stmt[4]) {
          ...
          pre.push('$block.f = (' + tmp_recv + ' = ' + recv + ')' + this.mid_to_jsid(stmt[2]) + ').call');
    ...
    //     }
    //     else {
    //       // pre.push(this.SELF + '.$m' + this.mid_to_jsid(st<D-/>mt[2]));
    ...
    if (stmt[3][1]) {
      ...
      pre.push('.apply(' + recv + ', ' + pre_splat_arg);
  ...
  generate_assign: function(stmt, o) {
    ...
    // a.b = c
      ...
      return this.generate_call(['call', stmt[1][1], stmt[1][2] + '=', [[stmt[2]]]]);
  ...
  generate_aref: function(stmt, o) {
    ...
    return this.generate_call(['call', stmt[1], '[]', [stmt[2][0]]]);
  },

  // [arefs, aset]
  generate_aset: function(aref, arg) {

    return this.generate_call(['call', aref[1], '[]=', [aref[2][0].concat([arg])]]);
  },
  ...
  generate_string: function(stmt, o) {
    ...
    if (stmt[1].length == 0) {
      ...
      if (stmt[1][0][0] == 'string_content') {
        ...
        // res.push(this.mid_to_jsid('to_s') + '(' + tmp_to_s + '))');
        // this.iseq_current.queue_temp(tmp_to_s);
        res.push(this.generate(stmt[1][0][1][1][0]) + this.mid_to_jsid('to_s') + '()');
      }
      else {
      ...
      for (var i = 0; i < stmt[1].length; i++) {
        ...
        if (part[0] == 'string_content') {
          ...
          // res.push(this.mid_to_jsid('to_s') + '(' + tmp_to_s + '))');
          // this.iseq_current.queue_temp(tmp_to_s);
          res.push(this.generate(part[1][1][0]) + this.mid_to_jsid('to_s') + '()');
        }
        else {
          res.push(this.SELF + '.i$' + part[1] + '.$to_s(self)');
        }
  ...
  generate_yield: function(stmt) {
    ...
    if (stmt[1][1]) {
      ...
      // no splat
      return block + '.call(' + args_res.join('') + ')';
    }
    // res.push(')');
