...
RubyGenerator.prototype = {
  ...
  generate_call: function(stmt) {
    ...
    if (stmt[1]) {
      ...
      res.push('(' + tmp_recv + ' = ');
      res.push(this.generate(stmt[1]));
      res.push(', ' + tmp_recv + '.$m' + this.mid_to_jsid(stmt[2]) + ' || ' + tmp_recv);
      res.push('.$M("' + stmt[2] + '"))(');
      res.push(tmp_recv + ', ');
      this.iseq_current.queue_temp(tmp_recv);
    }
    else { // no recv
      res.push('(' + this.SELF + '.$m' + this.mid_to_jsid(stmt[2]) + ' || ' + this.SELF);
      res.push('.$M("' + stmt[2] + '"))(');
      res.push(this.SELF + ', ');
    }
  ...
  generate_identifier: function(stmt, o) {
    ...
    if (local)
      ...
      return '(' + this.SELF + '.$m' + this.mid_to_jsid(stmt[1]) + ' || ' + this.SELF + '.$M("' + stmt[1] + '"))(' + this.SELF + ', ' + this.NIL + ')';
  ...
  generate_while: function(stmt) {
    ...
    res.push("while (true) {");
    // make sure while condition is still true
    res.push("if (!" + this.generate(stmt[2]) + ".$r){");
    res.push("break;}");
    // body condition
    ...
    return res.join("");
    // var tmp_assign = this.iseq_current.temp_local();
    // res.push(this.SELF + '.rbWhile(function(){');
    // res.push('return (' + tmp_assign + ' = '); 
    // // res.push((stmt[1] == 'while') ? 'return (' : 'return(!(');
    // res.push(this.generate(stmt[2]));
    // if (stmt[1] == 'while') {
    //   res.push(', ' + tmp_assign + ' !== ' + this.NIL + ' && ' + tmp_assign +
    //           ' !== false);')
    // }
    // else {
    //   res.push(', ' + tmp_assign + ' == ' + this.NIL + ' || ' + tmp_assign +
  ...
  generate_unary: function(stmt, o) {
    ...
    var code = "(" + tmp_recv + ' = ' + this.generate(stmt[2]) + ', ' + tmp_recv
              + '.$m' + this.mid_to_jsid(meth) + ' || ' + tmp_recv + '.$M("' + meth + 
              '"))(' + tmp_recv + ', ' + this.NIL + ')';
    this.iseq_current.queue_temp(tmp_recv);
  ...
  generate_begin: function(stmt) {
    ...
    // wrap it.
    res.push('if (!__err__.isa) { __err__ = rb_make_exception(__err__);}');
