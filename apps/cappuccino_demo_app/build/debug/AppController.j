...
var j=rb_funcall(rb_const_get_full(self.isa,'JSONObject'),'new:',d);
return rb_funcall(self,'puts:',rb_funcall(j,'name'));
});
...
self = arguments.callee.self || self;
rb_funcall(win,'<<',rb_funcall(self,'button:',rb_hash_new(ID2SYM('title'),"adam",ID2SYM('frame'),[50,50,300,24],ID2SYM('enabled'),false,ID2SYM('on_action'),rb_funcall_block(rb_const_get_full(self.isa,'Proc'),'new',function(){
self = arguments.callee.self || self;
...
rb_funcall(self,'puts:',"trying something");
return rb_funcall(self,'puts:',rb_funcall(self,'something'));
});
