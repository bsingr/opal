...
rb_funcall(self,s$ah,["KVB: '",(binding),"' is not exposed on object."].join(''));
}
if(!RTEST(ANDTEST(observable,key_path))){
rb_funcall(self,s$ah,["KVB: bad path/object for binding '",(binding),"' to '",(key_path),"'"].join(''));
}
rb_funcall(self,s$fo,binding);
rb_funcall(observable,s$bn,self,key_path,options,binding);
rb_funcall(rb_ivar_get(self, i$ad),'[]=',binding,VN.$h(_$bb, observable, _$bc, key_path, _$m, options, _$bd, binding));
...
rb_funcall(self,s$ah,['KVB: received notification for chnage of context ',(context)].join(''));
rb_funcall(self,s$fp,context);
}
});
rb_define_method(self,s$fp,function(self,_cmd,binding){
var dict = rb_funcall(self,s$fq,binding);
var obj = rb_funcall(dict,s$e,_$bb);
var path = rb_funcall(dict,s$e,_$bc);
var key = rb_funcall(dict,s$e,_$bd);
...
var obj = rb_funcall(rb_funcall(self, s$fs),s$e,_$bb);
var path = rb_funcall(rb_funcall(self, s$fs),s$e,_$bc);
var value = rb_funcall(self,s$at,rb_funcall(rb_funcall(self, s$fs),s$e,_$bd));
...
return rb_funcall(rb_ivar_get(self, i$ad),s$e,binding);
});
