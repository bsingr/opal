class Vienna::ObjectiveJ
  ...
  rule
    ...
    class_name_declaration:
    	  class_with_superclass                                                         { result = Vienna::Node.new(',', val[0], nil) }
    	| class_with_superclass '<' inherited_protocols '>'                             { result = Vienna::Node.new(',', val[0], Vienna::Node.new(',', val[2], nil)) }
    	| class_with_superclass '(' category_name ')'                                   { result = Vienna::Node.new(',', val[0], Vienna::Node.new(',', nil, val[2])) }
    	| class_with_superclass '<' inherited_protocols '>' '(' category_name ')'       { result = Vienna::Node.new(',', val[0], Vienna::Node.new(',', val[2], val[5])) }
    	;
